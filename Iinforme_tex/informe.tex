...
\begin{figure}[H]
    ...
    \label{fig:cap_card}
\end{figure}
\textit{Análisis Técnico}: La tarjeta proyecta tres elementos clave: (i) la insignia de valor \textit{Value Score} (\textit{Precio Justo}, \textit{Top Calidad/Precio}), (ii) las barras de progreso desglosando la coincidencia por submodelo (MAUT 63\%, SVD 44\%, Contenido 74\%), y (iii) la barra inferior con botones independientes: a la izquierda el enlace directo parametrizado a **Amazon** (\texttt{https://www.amazon.com/s?k=Marca+Modelo}) y a la derecha el disparador del gráfico modal de precios.
...
\begin{figure}[H]
    ...
    \label{fig:cap_chart}
\end{figure}
\textit{Análisis Técnico}: Utilizando la librería **Chart.js** sobre un elemento Canvas HTML5, el modal dibuja la curva de fluctuación del precio en los últimos 6 meses a partir de la propiedad JSON \texttt{price\_history\_json}. Además, despliega tarjetas resumen con el Precio Actual, Mínimo Histórico, Máximo Histórico y Promedio a 6 meses.
...
\end{figure}
\textit{Análisis Técnico}: Al marcar casillas de verificación como \textit{Pantalla OLED}, \textit{Aceleradores CUDA} o \textit{RAM $\ge 16$GB}, la máscara binaria $M_{rules}$ descarta de inmediato los modelos no aptos. La interfaz informa cuántas laptops superaron el filtro (ej. 702 de 1,273 modelos en el catálogo ampliado).
...
\end{figure}
\textit{Análisis Técnico}: La vista proporciona una tabla de datos completa con ordenamiento por cualquier columna y un campo de búsqueda predictiva en tiempo real que filtra por marca o nombre de modelo.
...
\begin{figure}[H]
    ...
    \label{fig:cap_eval}
\end{figure}
\textit{Análisis Técnico}: Diseñado para la auditoría docente durante la sustentación, este panel consulta el endpoint \texttt{/api/metrics} y proyecta en vivo las curvas de $RMSE$, $Precision@K$, $Recall@K$ y la comparación de $CSR@K$ frente al modelo SVD colaborativo aislado.
